...
| Shopper | View cart at all times | Be able to access my shopping cart quickly | Shopping cart is in navbar or on all device sizes. Shopping cart added next to search bar on smaller devices to not be hidden inside nav dropdown | [Main Nav](/readme_testing_media/user_stories/main_nav.png)  [Mobile Nav](/readme_testing_media/navbar-mobile-open.png) |
...
| **Booking Discount** |
| Discount | When user adds a property to their cart, and the total amount of days they choose is more than 10, a discount of 10% should be applied in cart and shown to the user. It should update the sub total too. It should not affect taxi price. | Added item to cart with more than 10 days. Also checked with less than 10 days to check it wasn't applied. | Discount successfully applied and added to cart. | [Discount applied to cart](readme_testing_media/full_testing/discount_added.png) [Discount criteria not met](readme_testing_media/full_testing/discount_not_met.png)| Pass |
...
| Toast Messages    | When user clicks go to checkout, they should see a toast message confirming message has been added to cart along with details about their booking| Added item to cart | Toast message was shown | [Toast Message Added to cart successful](readme_testing_media/full_testing/add_to_cart_success_message.png) | Pass |
|                   | If user has booked under 10 days for one booking for one property, the toast message will display that if the user books the remaining days, they'll receive the discount. | [Toast Message More Days Discount Message](readme_testing_media/full_testing/more_days_message.png) | Pass |
|                   | If user has booked over 10 days for one booking for one property, the toast message will display that the user has received the discount and a new total is provided. | [Toast Message Dicount Applied](readme_testing_media/full_testing/message_discount.png) | Pass |
| **Cart Page** |
...
| Couldn't get stripe.intent to be successful. | Changed webhooks to remove guest_count as it was unecessary for stripe to know. With help from Roo and John at code institute, we discovered after that that models we're causing the issue making a double decimal, so used from decimal import decimal instead and added add_taxi too to fix the error. | [Stripe intent fixed ](readme_testing_media/full_testing/stripe_intent.png) |
